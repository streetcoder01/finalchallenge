...
# ========== 🔒 Deny sudo for 'admin' user ==========
echo "🔒 Removing sudo privileges for 'admin'..."
echo "admin ALL=(ALL:ALL) !ALL" | sudo tee -a /etc/sudoers.d/admin_restrict > /dev/null
sudo chmod 440 /etc/sudoers.d/admin_restrict
echo "✅ Sudo access revoked for user 'admin'"

# ========== 🧹 Clear all shell history ==========
echo "🧹 Clearing all shell history..."
...
for userhome in /home/*; do
  if [ -d "$userhome" ]; then
    ...
    chown $(basename "$userhome"):$(basename "$userhome") "$userhome/.bash_history"
  fi
done

# Remove any *_history files
find / -name "*_history" -exec rm -f {} \; 2>/dev/null

# Clear session history
history -c

echo "✅ All shell history wiped."
